...
export async function getServiceStatus() {
  try {
    ...
    const clientAvailable = !!visionClient || (
      (!!process.env.GOOGLE_APPLICATION_CREDENTIALS || !!process.env.GOOGLE_CLOUD_VISION_API_KEY) &&
      !!process.env.GCS_BUCKET_NAME
    );
    const hasBucket = !!(process.env.GCS_BUCKET_NAME || process.env.GCS_BUCKET);
    return {
      service: SERVICE_NAME,
      available: ENABLED && clientAvailable,
      enabled: ENABLED,
      hasCredentials: !!process.env.GOOGLE_APPLICATION_CREDENTIALS || !!process.env.GOOGLE_CLOUD_VISION_API_KEY,
      hasBucket
    };
  } catch (error) {
    return {
