...
// 설정
const CONFIG = {
  popplerPath: process.env.POPPLER_PATH || '/usr/bin',
  ...
  groundTruthDir: path.join(process.cwd(), 'src/rag/case_sample'),
  ...
  maxRetries: 3,
  maxPagesPerCall: 5,  // 10에서 5로 줄임 (대용량 이미지 처리)
  batchDelay: 65000,
...
function getOpenAI() {
  if (!openai) {
    const config = {
      ...
      timeout: 600000,  // 10분으로 증가 (대용량 이미지 처리)
...
function getCaseInfo(caseSets, caseNum) {
  if (caseSets.details && caseSets.details[caseNum]) {
    ...
    // Linux 경로로 변환
    if (detail.pdfFolder && detail.hasPdf) {
      pdfFolder = path.join('/home/user/VNEXSUS_reports_pdf/sample_pdf/sample_pdf', detail.pdfFolder);
    }
...
async function pdfToImages(pdfFolder) {
  ...
  for (const pdfFile of pdfFiles) {
    ...
    const pdftoppm = path.join(CONFIG.popplerPath, 'pdftoppm');
