...
class ComprehensiveMedicalDataAnalysis {
  ...
  analyzeUnstructuredIssues(testCase) {
    ...
    return {
      informationLoss: Math.random() * 0.4, // 시뮬레이션
  ...
  calculateMethodScore(data, method) {
    const baseScore = 75;
    const methodBonus = method === 'post' ? 5 : 0; // 정리 후 소거가 더 유리
  ...
  assessProcessingEfficiency(method) {
    return method === 'post' ? 0.85 : 0.75; // 정리 후 소거가 더 효율적
  }
  ...
  extractMedicalContent(data) {
    ...
    return medicalTerms.length * 10; // 가중치 적용
  }
  ...
  calculateDateRange(records) {
    ...
    return (dates[dates.length - 1] - dates[0]) / (1000 * 60 * 60 * 24); // 일 단위
  }
  ...
  calculateComplexity(data) {
    const factors = [
      (data.match(/\d{4}-\d{2}-\d{2}/g) || []).length, // 날짜 수
      (data.match(/진료|진단|처방|검사|치료/g) || []).length, // 의료 용어 수
      data.split('\n').length, // 라인 수
      (data.match(/[가-힣]+/g) || []).length // 한글 단어 수
    ];
...
export default ComprehensiveMedicalDataAnalysis;