...
class Preprocessor {
  constructor() {
    ...
    this.datePatterns = [
      ...
      /(\d{1,2})월\s*(\d{1,2})일/g,
      // 레이블+YYYYMMDD 붙여쓰기 형식 (예: 진료일: 20250310)
      // 의료 레이블 뒤에 오는 8자리 숫자를 YYYY MM DD로 파싱
      /(?:작성일|진료일|수술일|검사일|처방일|퇴원일|입원일|처치일|투약일)[:\s]*(\d{4})(\d{2})(\d{2})(?!\d)/g,
      // 레이블+YYYY-MM-DD 명시 형식 (예: 작성일 2022-08-22 — 기존 패턴으로도 추출되지만 레이블 우선 파싱)
      /(?:작성일|진료일|수술일|검사일|처방일|퇴원일|입원일|처치일|투약일)\s+(\d{4})[.\-](\d{1,2})[.\-](\d{1,2})/g
    ];
