...
let postProcessor;
try {
    // CommonJS로 ES module 로드
    postProcessor = require('./index.js');
} catch (e) {
    console.log('postProcessor require 실패, 동적 import 시도...');
}
...
async function processSingleCase(caseInfo, processor) {
    ...
    try {
        ...
        const result = await processor.processForMainApp(content, options);
...
async function main() {
    ...
    // PostProcessor 로드
    console.log('\n📦 PostProcessor 로드 중...');
    const processor = await loadPostProcessor();
    if (!processor || !processor.processForMainApp) {
        console.error('❌ PostProcessor 로드 실패');
        process.exit(1);
    }
    console.log('✅ PostProcessor 로드 완료');
    ...
    for (const caseInfo of allCases) {
        const result = await processSingleCase(caseInfo, processor);
